TITLE Mod file for component: Component(id=ka_ib type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=ka_ib type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=ka_ib type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=ka_ib_m_tau_tau)
    ...
    if (h_timeCourse_V   < ( -63.0 ))  { 
        h_timeCourse_t = ( 2.6 * 0.5 / ((exp ((  h_timeCourse_V  + 46 )/5)) + (exp (( - h_timeCourse_V  - 238 )/37.5))) ) *  h_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
        h_timeCourse_t = ( 2.6 * 9.5) *  h_timeCourse_TIME_SCALE ? evaluable cdv
    }
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=ka_ib_h_tau_tau)
