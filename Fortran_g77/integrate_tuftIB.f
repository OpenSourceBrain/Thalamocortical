...
c numcomp = number of compartments, including 6 in the axon.
     ...
     &    gjtable_mix (totaxgj_mix,4)
       ...
       REAL*8 Z, Z1, Z2, Z3, z4, curr(numcomp_p,numcell),c(numcomp_p)
...
c CINV is 1/C, i.e. inverse capacitance
     ...
     x mcat(numcomp_p,numcell),
     x hcat(numcomp_p,numcell),mcal(numcomp_p,numcell),
     ...
     x gampa(numcomp_p,numcell),
     x gnmda(numcomp_p,numcell),ggaba_a(numcomp_p,numcell),
     x cafor(numcomp_p)
...
c below are for calculating the partial derivatives
     ...
     x dfmnaf_dmnaf(numcomp_p),
     x dfmnaf_dv(numcomp_p),dfhnaf_dhnaf(numcomp_p),
     x dfmnap_dmnap(numcomp_p), dfmnap_dv(numcomp_p),
     x dfhnaf_dv(numcomp_p),
     x dfmkdr_dmkdr(numcomp_p),dfmkdr_dv(numcomp_p),
     ...
     x dfmcat_dmcat(numcomp_p),
     x dfmcat_dv(numcomp_p),dfhcat_dhcat(numcomp_p),
     x dfhcat_dv(numcomp_p),
     x dfmcal_dmcal(numcomp_p),dfmcal_dv(numcomp_p),
     x dfmar_dmar(numcomp_p),
     x dfmar_dv(numcomp_p),dfmkahp_dchi(numcomp_p),
     x dfmkahp_dmkahp(numcomp_p), dt2

      REAL*8 vL(numcomp_p),vk(numcomp_p),vna,var,vca,vgaba_a
      REAL*8 xopen(numcomp_p),gamma(numcomp_p),gamma_prime(numcomp_p)
...
c Consistent set of units: nF, mV, ms, nA, microS
      ...
      REAL*8 GKDR(numcomp_p),GKA(numcomp_p),GAR(numcomp_p)
      REAL*8 GKC(numcomp_p),GKAHP(numcomp_p),GCAL(numcomp_p)
      REAL*8 GK2(numcomp_p),GKM(numcomp_p),GNAP(numcomp_p),CDENS
...
c       gnaf_dens =  5.d0
c       gnaf_dens = 10.d0
!        gnaf_dens = 15.d0 ! uncommented in original fortran
        do i=0 , 18
        gnaf_dens(i) = 15.d0
        end do
        gnaf_dens(0) = 450.d0
...
!        gkc_dens = 0.25d0
        ...
        gkc_dens(i) = 0.25d0
         ...
         gkc_dens(0) =  0.d0 ! not present in original code put indicated in Appendix A
         gkc_dens(1) =  8.00d0
         gkc_dens(2) =  8.00d0
         gkc_dens(5) =  8.00d0
         gkc_dens(6) =  8.00d0
