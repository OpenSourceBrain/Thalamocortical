...
! From dienf.                  
     ...
     &  mar)
       ...
       parameter (numcomp_p = 137)  ! should be compat. with calling prog
...
c below are for calculating the partial derivatives
     ...
     x dfmkahp_dmkahp(numcomp_p), dt2, outrcd(20)
      ...
      REAL*8 vL(numcomp_p),vk(numcomp_p)
      REAL*8 vna,var,vca,vgaba_a,Z,Z1,Z2
      INTEGER  K0, K1, K2, NEIGH(numcomp_p,11), NNUM(numcomp_p)
...
c gamma is function of chi used in calculating KC conductance
       ...
       REAL*8 gca_high(numcomp_p), A, BB1, BB2
...
c ? initialize membrane state variables?
        ...
        v(i,j) = VL(i)
         chi(i,j) = 0.d0
        k1 = idnint (4.d0 * (-85.d0 + 120.d0)) ! Traub sets to -85 in orig fort- ask Traub if meant to
c        k1 = idnint (4.d0 * (v(i,j) + 120.d0))
...
c     mar=alpham_ar(k1)/(alpham_ar(k1)+betam_ar(k1))
      mar(i,j) = .25d0
         ...
         open(i) = 0.d0
         gkm(i) = 0.d0 ! set to 0 in orig Traub fort- ask Traub if meant to
         gkahp(i) = 0.d0 ! set to 0 in orig Traub fort- ask Traub if meant to
      end do
...
c                 gka = 0.d0
c                 gk2 = 0.d0
cxx                  gkm = 0.d0 ! set to 0 in orig Traub fort- ask Traub if meant to
c                 gkc = 0.d0
cxx                  gkahp = 0.d0 ! set to 0 in orig Traub fort- ask Traub if meant to
c                 gcat = 0.d0
c                 gcaL = 0.d0
c                 gar = 0.d0
...
c Consistent set of units: nF, mV, ms, nA, microS
        ...
        parameter (numcomp_p = 137)
        integer numcomp/numcomp_p/
        REAL*8 C(numcomp_p),GL(numcomp_p),
     &   GAM(0:numcomp_p,0:numcomp_p),GNAF(numcomp_p),GCAT(numcomp_p)
        REAL*8 GKDR(numcomp_p),GKA(numcomp_p)
        REAL*8 GKC(numcomp_p),GKAHP(numcomp_p),GCAL(numcomp_p)
        REAL*8 GK2(numcomp_p),GKM(numcomp_p)
        REAL*8 GNAP(numcomp_p),GAR(numcomp_p), CDENS, RM_AXON
        REAL*8 JACOB(numcomp_p,numcomp_p),RI_SD,RI_AXON,RM_SD
