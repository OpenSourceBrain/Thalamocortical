...
! From baskn.f in supergj.f
       ...
       SUBROUTINE integrate_deepaxax (O, time, numcell, V, curr,
...
c CINV is 1/C, i.e. inverse capacitance
     ...
     x cafor(numcomp_p), ggaba_a(numcomp_p,numcell),
...
c other state variables.
     ...
     x fmka(numcomp_p),fhka(numcomp_p),fmk2(numcomp_p),fhk2(numcomp_p),
...
c Consistent set of units: nF, mV, ms, nA, microS
        ...
        REAL*8 GKAHP(numcomp_p),GCAL(numcomp_p),GAR(numcomp_p)
        ...
        REAL*8 RES, RINPUTi, ELEN(numcomp_p)
